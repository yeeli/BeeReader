...
ipcMain.on('/entries', (event, arg, ktm) => {
  ...
  Entry.where({ "entries.account_id": arg.account }).select("entries.*", "streams.title as stream_title").join('streams', {'streams.id': 'entries.stream_id'}).orderBy('created_at', 'desc').then(res => {
